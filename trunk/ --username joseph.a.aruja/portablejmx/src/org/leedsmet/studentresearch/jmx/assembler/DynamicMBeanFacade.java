...
public class DynamicMBeanFacade implements DynamicMBean {
	...
	public DynamicMBeanFacade(Object managedResource, String description)
			throws DynamicMBeanFacadeException {
		if (managedResource == null)
			throw new DynamicMBeanFacadeException(
					"The managedResource reference cannot be null!");
		...
		_mbi = new MBeanInfo(_managedResource.getClass().getName(),
				description, new MBeanAttributeInfo[0],
	...
	public synchronized void addAttribute(String name, String description)
		...
		System.out
				.println("DynamicMBeanFacade.addAttribute(): INFO: Looking for attribute \'"
						+ name + "\'");
		...
		for (int aa = 0; aa < methods.length; aa++) {
			...
			if (methodName.equals("get" + name)) {
				...
				isWritable = true;
				System.out.println("\tSetter found: \'" + methodName
						+ "\', type = \'" + type + "\'");
		...
		if (!isReadable && !isWritable)
			throw new DynamicMBeanFacadeException("Attribute \'" + name
					+ "\' not found.");

		MBeanAttributeInfo newAttribute = new MBeanAttributeInfo(name, type,
				description, isReadable, isWritable, isIs);
		System.out.println("\tMetadata: [name=\'" + name + "], [type=\'" + type
				+ "\'], [isReadable=" + isReadable + "], [isWritable="
				+ isWritable + "], [isIs=" + isIs + "]");
		...
		MBeanInfo newMbi = new MBeanInfo(_mbi.getClassName(), _mbi
				.getDescription(), newAttributes, _mbi.getConstructors(), _mbi
				.getOperations(), _mbi.getNotifications());
		_mbi = newMbi;
	}
	...
	public synchronized void removeAttribute(String name)
		...
		MBeanAttributeInfo[] attributes = _mbi.getAttributes();
		MBeanAttributeInfo[] newAttributes = new MBeanAttributeInfo[attributes.length - 1]; // optimistic
		int newIndex = 0;
		try {
	...
	public synchronized void addOperation(String name, String description)
		...
		System.out
				.println("DynamicMBeanFacade.addOperation(): INFO: Looking for operation \'"
						+ name + "\'");
		Class current = _managedResource.getClass();
		Method[] methods = current.getMethods();
		...
		for (int aa = 0; aa < methods.length; aa++) {
			if (methods[aa].getName().equals(name)) {
				System.out.println("\tFound method: \'" + methods[aa].getName()
						+ "\'");
		...
		MBeanOperationInfo newOp = new MBeanOperationInfo(description,
				theMethod);
		MBeanOperationInfo[] ops = _mbi.getOperations();
		MBeanOperationInfo[] newOps = new MBeanOperationInfo[ops.length + 1];
		System.arraycopy(ops, 0, newOps, 0, ops.length);
		newOps[ops.length] = newOp;
		// Recreate MBeanInfo, with new operation metadata array
		MBeanInfo newMbi = new MBeanInfo(_mbi.getClassName(), _mbi
				.getDescription(), _mbi.getAttributes(),
				_mbi.getConstructors(), newOps, _mbi.getNotifications());
		_mbi = newMbi;
	}
	...
	public synchronized void removeOperation(String name)
		...
		MBeanOperationInfo[] ops = _mbi.getOperations();
		MBeanOperationInfo[] newOps = new MBeanOperationInfo[ops.length - 1]; // optimistic
		int newIndex = 0;
		try {
	...
	public synchronized void addNotification(String type, String description)
		...
		MBeanNotificationInfo notif = new MBeanNotificationInfo(
				new String[] { type }, "javax.management.Notification",
				description);

		MBeanNotificationInfo[] notifs = _mbi.getNotifications();
		MBeanNotificationInfo[] newNotifs = new MBeanNotificationInfo[notifs.length + 1];
		...
		MBeanInfo newMbi = new MBeanInfo(_mbi.getClassName(), _mbi
				.getDescription(), _mbi.getAttributes(),
				_mbi.getConstructors(), _mbi.getOperations(), newNotifs);
		_mbi = newMbi;
	}
	...
	/**
	 * to handle getting Class objects for primitive types.
	 * @param className
	 * @return
